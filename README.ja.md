...
`.env` が既定の `host.containers.internal` を使っている場合、現在は Windows + Podman でその host alias が使えない時に、生成される runtime config が Podman-machine gateway へ自動解決します。`.\scripts\doctor.ps1` も、実際に使われる Ollama `/api/tags` endpoint へ届かなければ fail します。

固定したい Ollama endpoint がある場合だけ、`.env` の `OPENCLAW_OLLAMA_BASE_URL` を明示的に置き換えてください。
